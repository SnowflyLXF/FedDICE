...
class NeuralDice(object):
  ...
  def __init__(
    ...
    self._theta_network.create_variables()
    ...
    self._theta_regularizer = theta_regularizer
  ...
  def _get_log_prob(self, policy_network, env_step):
    if self._categorical_action and self._num_samples is None:
      action = np.zeros((248, 1))
      p = policy_network(env_step.observation)[0].prob(action)
      for i in range(len(p)):
        action[i, 0] = random.choices([0,1], weights=[p[i], 1-p[i]])[0]
      return policy_network(env_step.observation)[0].log_prob(action)
    else:
      a = self._theta_network(env_step.observation)[0].sample(self._num_samples)
      return tf.reduce_mean(policy_network(env_step.observation)[0].log_prob(a), axis=0)
  ...
  def _get_average_value(self, network, env_step):
    if self._solve_for_state_action_ratio:
      ...
      if self._categorical_action and self._num_samples is None:
        ...
        #     axis=1)
        actions = self._theta_network(env_step.observation)[0].sample(self._num_samples)
  ...
  def train_loss(self, initial_env_step, env_step, next_env_step):
    ...
    initial_log_prob = self._get_log_prob(self._theta_network, initial_env_step)
    next_log_prob = self._get_log_prob(self._theta_network, next_env_step)
    theta_loss = -(1-self._gamma) * initial_log_prob * initial_nu_values - self._gamma * zeta_values * next_nu_values * next_log_prob
    ...
    return nu_loss, zeta_loss, lam_loss, theta_loss
  ...
  def train_step(self, initial_env_step: dataset_lib.EnvStep,
    ...
    with tf.GradientTape(
      ...
      tape.watch(self._theta_network.variables)
      
      nu_loss, zeta_loss, lam_loss, theta_loss = self.train_loss(initial_env_step, env_step, next_env_step)
      ...
      zeta_loss += self._zeta_regularizer * self._orthogonal_regularization(
          self._zeta_network)
      theta_loss += self._theta_regularizer * self._orthogonal_regularization(
        self._theta_network)
    ...
    if update_actor:
      theta_grads = tape.gradient(theta_loss, self._theta_network.variables)
      theta_grad_op = self._theta_optimizer.apply_gradients(
          zip(theta_grads, self._theta_network.variables))


    return (tf.reduce_mean(nu_loss), tf.reduce_mean(zeta_loss),
            tf.reduce_mean(lam_loss), tf.reduce_mean(theta_loss))
