...
# from dice_rl.utils.adamirror import AdamirrorOptimizer
...
flags.DEFINE_string('load_dir', '/home/snowfly/dice/dice_rl/tests/testdata', 'Directory to load dataset from.')
...
flags.DEFINE_integer('seed', 1, 'Initial random seed.')
...
flags.DEFINE_float('alpha', 0.5, 'How close to target policy.')
...
flags.DEFINE_float('policy_learning_rate', 0.0005, 'Learning rate for policy.')
...
flags.DEFINE_integer('eval_trajectory', 50,
...
flags.DEFINE_integer('opt_interval', 50,
...
flags.DEFINE_integer('warmup_step', 20000,
...
#   avg_rew = optimizer.evaluate_policy(env, num_traj*2, traj_len, target_policy)
  # target_policy = get_target_policy(load_dir, env_name, tabular_obs, alpha=1.0)
  filename = "/home/snowfly/dice/dice_rl/results/records/cartpole_"+str(policy_learning_rate)+"_"+str(eval_interval)+".csv"
  with open(filename, "w") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['step', 'primal', 'dual'])
  # running_losses = []
  running_rewards = []
    ...
    if (step-warmup_step) % opt_interval == 0 and step >= warmup_step:
        ...
        if (step-warmup_step) % eval_interval == 0:
            ...
            with open(filename, "a") as csvfile:
              writer = csv.writer(csvfile)
              writer.writerow([step, primal, dual, reward, np.mean(running_rewards)])
...
# def get_env(env_name, tabular_obs=False, env_seed=0):
#     if env_name == 'grid':
#         env = navigation.GridWalk(tabular_obs=tabular_obs)
#         env.seed(env_seed)
#     elif env_name == 'cartpole':
