...
class ValueNetwork(network.Network):
  ...
  def predict(self, inputs):
    out, _ = self.call(inputs)

    return out


class LinearValueNetwork(network.Network):
  """Creates a critic network."""
  ...
  def __init__(self,
               ...
               last_kernel_initializer=None,
               output_dim=None,
               name='LinearValueNetwork'):
    """Creates an instance of `LinearValueNetwork`.
    ...
    Args:
      ...
      name: A string representing name of the network.
    """
    super(LinearValueNetwork, self).__init__(
        input_tensor_spec=input_tensor_spec,
        state_spec=(),
        name=name)
    ...
    self._fc_layers = tf.keras.layers.Dense(
            output_dim or 1,
            activation=output_activation_fn,
            kernel_initializer=last_kernel_initializer,
            name='value')
  ...
  def call(self, inputs, step_type=(), network_state=(), training=False):
    ...
    joint = tf.concat(processed_inputs, -1)
    joint = self._fc_layers(joint, training=training)
